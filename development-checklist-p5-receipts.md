...
更新日期：2026-09-14。P5-1、P5-2 和 P5-3 均已完成工程实现、云端部署和受控真实验收。正式验收活动已贯通供应商受理、送达、打开、点击、公开退订、工作区抑制和活动统计闭环。
...
- [x] P5-2 使用两个受控真实邮箱完成客户端退订入口、GET 安全跳转、页面确认和抑制闭环验收。
- [ ] P5-2 非阻塞扩展回归：由明确支持 RFC 8058 的客户端真实触发 one-click POST，并创建退订后的新活动复核发送排除；当前自动化测试已覆盖两项行为。
- [x] P5-3 活动级送达、退信、投诉、退订、打开和点击统计界面，并明确送达率与行为率分母。
- [x] P5-3 通道级行为追踪配置、发送运行冻结、DirectMail HTML/标签/ClickTrace 参数和退订链接排除追踪。
- [x] P5-3 逐收件人结果明细及等待回执、送达、退信、投诉、退订、打开和点击筛选。
- [x] P5-3 已完成云端迁移、Edge Function 与生产前端部署，并以真实邮件完成送达、打开、点击和公开退订验收。
...
- P5-2 新增令牌、邮件头/页脚、数据库事务和公开页面测试；P5-3 新增追踪请求、HTML 安全转义、统计聚合、权限和结果筛选测试。
- 最终回归 `npm test` 共 92 项通过，浏览器端到端测试共 8 项通过，`npm run lint`、`npm run typecheck`、格式检查和生产构建通过。
...
9. [x] 未主动向外部地址制造硬退信、投诉或供应商退订；这些负向状态由自动化测试覆盖，P5-2 公开退订另以两个受控真实地址完成验收。
...
12. [x] `edm-unsubscribe` v3、`edm-directmail-worker` v6 和 Vercel 生产部署均为 Active/Ready，正式域名已绑定；兼容修复部署 `dpl_4T8FpYSQ9K57KT7EyFyKG4mpUVUB` 已上线，Worker 无效调用返回 401，确认配置加载后仍执行数据库业务鉴权。
13. [x] 用户确认两个受控真实邮箱的退订验收通过。邮箱客户端顶部“取消订阅”入口实际以 GET 打开 API 地址，兼容修复后跳转至公开确认页；两个地址确认退订后，云端共记录 2 条 `public_page` 事件，分别关联 2 个来源任务、2 条抑制记录、2 个联系人退订状态和 2 个来源任务反馈投影，未记录客户正文或邮箱地址。
14. [ ] 非阻塞扩展回归：使用明确支持 RFC 8058 的客户端触发标准 one-click POST，并在退订后创建新活动，确认两个地址均以 `recipient_ineligible` 跳过且不进入供应商调用。当前尚无退订后的新投递任务，不能把数据库门禁的自动化覆盖写成真实发送证据。
15. [x] 已应用云端迁移 `20260913152515_p5_campaign_statistics_tracking`（本地源文件为 `20260913150707_p5_campaign_statistics_tracking.sql`），仅扩展 `edm` / `edm_private`；`edm-directmail-worker` v7、测试函数 v6 和 Vercel 生产部署 `dpl_8KJLZZWaP6Tzbna7REhZ2VjwQ7U1` 均已上线。
16. [x] 当前通道已启用行为追踪标签 `pixel_edm_tracking`。正式验收活动 `52c921f5-862c-4b68-9154-a29ce54c0cba`、运行 `cf604f76-4b73-4bb2-9b31-7fa945e35597` 和任务 `00a480e5-cbb6-4688-96c2-b0559e3768c3` 已由 DirectMail 接受，EnvId 为 `600000353321881164`。
17. [x] 验收发现 EventBridge 规则仍指向早期工作区的旧通道。已轮换当前通道 Webhook 令牌、把规则目标修正到当前通道并保持启用；随后仅将该 EnvId 对应的 3 条已知真实事件定向归属并重放。投递成功、打开、点击事件均为 `applied`，且精确关联同一运行和任务，未改动其他历史事件。
18. [x] 线上活动列表和统计明细均显示已受理 1、已送达 1、已打开 1、已点击 1，送达率、打开率和点击率均为 100%；“已打开”和“已点击”筛选均命中真实收件任务。
19. [x] 用户通过本次真实邮件的公开确认页完成退订。云端仅产生 1 条 `public_page` 退订事件和 1 条工作区抑制，联系人及来源任务反馈均更新为 `unsubscribed`；活动统计显示已退订 1（100%），“已退订”筛选命中同一任务。
