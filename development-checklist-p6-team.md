...
更新日期：2026-09-18。P6-0 至 P6-3 已完成工程实现，P6-4 已完成审计与自动化验收；生产域名下使用第二个真实账号接受邀请的人工验收仍待完成。Supabase 继续复用 content-up 项目，业务对象只落在 `edm` / `edm_private`；本批未修改 `aigc`、共享 Auth 触发器或既有迁移历史。

## 当前完成状态

- [x] P6-0 邀请、成员生命周期、角色、移除和所有权规则已固化。
- [x] P6-1 数据库迁移、受控 RPC、RLS、权限和并发保护已完成并应用至云端。
- [x] P6-2 团队成员页及邀请、角色和成员管理界面已完成。
- [x] P6-3 `/invite/[token]` 邀请接受流程及防冒领、一次性消费保护已完成。
- [x] P6-4 审计、数据库自动化测试、类型检查、lint、格式检查和生产构建通过。
- [ ] P6-4 使用第二个真实账号在生产域名完成一次邀请链接接受的人工验收；该项需要额外已验证邮箱，不影响已完成的代码、数据库和自动化验收。
